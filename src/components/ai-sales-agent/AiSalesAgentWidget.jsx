...
function AiSalesAgentWidget() {
  ...
  return <div className="ai-sales-widget">
    ...
    {isOpen && !isMinimized && <section id="novahaus-ai-sales-widget-panel" className="ai-sales-widget-panel" role="dialog" aria-label="NOVAHAUS AI Growth Consultant">
      ...
      <div className="ai-sales-widget-footer"><span>Local session · Refresh-safe</span><button type="button" onClick={handleReset}>Reset</button></div>
    </section>}
    <button type="button" className={`ai-sales-widget-trigger ${isOpen ? 'is-open' : ''}`} aria-expanded={isOpen && !isMinimized} aria-controls="novahaus-ai-sales-widget-panel" onClick={isOpen && !isMinimized ? () => setIsOpen(false) : openWidget}><BrandLogo className="ai-sales-widget-trigger-logo" /><span><strong>NOVAHAUS AI</strong><small>{isOpen ? 'Close conversation' : 'Start a conversation'}</small></span><i aria-hidden="true">↗</i></button>
  </div>
}
