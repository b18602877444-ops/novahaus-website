...
function AiSalesAgentPage() {
  ...
  return <div className="sales-agent-page"><header className="sales-agent-header"><a href="/" aria-label="NOVAHAUS home"><BrandLogo className="sales-agent-logo" /></a><div><span>NOVAHAUS Growth Consultant</span><small>Guided commercial discovery · Local session</small></div><button type="button" className="sales-agent-reset" onClick={handleReset}>Reset conversation</button></header><main className="sales-agent-main">
    <section className="sales-agent-intro"><span className="sales-agent-eyebrow">GROWTH CONSULTANT / V1.0</span><h1>Turn the next<br /><em>conversation into clarity.</em></h1><p>A focused, consultative conversation for teams choosing the right NOVAHAUS engagement, with clear boundaries and a useful next step.</p><div className="sales-agent-flow"><span>Discover</span><i>→</i><span>Qualify</span><i>→</i><span>Recommend</span><i>→</i><span>Move forward</span></div></section>
    <section className="sales-agent-workspace" aria-label="NOVAHAUS Growth Consultant"><aside className="sales-agent-context"><span className="sales-agent-eyebrow">Conversation state</span><h2>{hasLead ? 'A clearer next step.' : 'Start with the business.'}</h2><p>{hasLead ? 'Your context has been saved locally and the next actions are ready.' : 'Start with your customer type. The consultant will ask only for the context it needs.'}</p><LeadTemperature qualification={conversation.qualification} />{showRecommendations && <div className="sales-agent-sidebar-recommendations"><span>Recommended path</span>{conversation.recommendations.map((recommendation) => <strong key={recommendation.serviceId}>{recommendation.title}</strong>)}</div>}<p className="sales-agent-privacy">Your conversation stays in this browser. This version does not send data to an external AI, CRM or email service.</p></aside>
