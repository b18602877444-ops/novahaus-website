...
function AiSalesAgentPage() {
  ...
  const handleProposal = () => { if (!hasLead) { sendMessage('Generate an Initial Proposal'); return }; const payload = createProposalPayload(); saveProposalPrefill(payload); trackAssessmentEvent('sales_agent_proposal_prefill_created', { conversationId: payload.conversationId }); window.location.assign('/proposal-builder/') }
  const handleReset = () => { if (window.confirm('Reset this conversation and clear its locally stored lead data?')) reset() }

  return <div className="sales-agent-page"><header className="sales-agent-header"><a href="/" aria-label="NOVAHAUS home"><BrandLogo className="sales-agent-logo" /></a><div><span>NOVAHAUS AI Sales Consultant</span><small>Guided commercial discovery · Local session</small></div><button type="button" className="sales-agent-reset" onClick={handleReset}>Reset conversation</button></header><main className="sales-agent-main">
    <section className="sales-agent-intro"><span className="sales-agent-eyebrow">AI SALES CONSULTANT / V1.0</span><h1>Turn the next<br /><em>conversation into clarity.</em></h1><p>A focused, consultative conversation for teams choosing the right NOVAHAUS engagement, with clear boundaries and a useful next step.</p><div className="sales-agent-flow"><span>Discover</span><i>→</i><span>Qualify</span><i>→</i><span>Recommend</span><i>→</i><span>Move forward</span></div></section>
    <section className="sales-agent-workspace" aria-label="NOVAHAUS AI Sales Consultant"><aside className="sales-agent-context"><span className="sales-agent-eyebrow">Conversation state</span><h2>{hasLead ? 'A clearer next step.' : 'Start with the business.'}</h2><p>{hasLead ? 'Your context has been saved locally and the next actions are ready.' : 'Start with your customer type. The consultant will ask only for the context it needs.'}</p><LeadTemperature qualification={conversation.qualification} />{showRecommendations && <div className="sales-agent-sidebar-recommendations"><span>Recommended path</span>{conversation.recommendations.map((recommendation) => <strong key={recommendation.serviceId}>{recommendation.title}</strong>)}</div>}<p className="sales-agent-privacy">Your conversation stays in this browser. This version does not send data to an external AI, CRM or email service.</p></aside>
      <div className="sales-agent-chat-card"><div className="sales-agent-chat-header"><div><span>Conversation</span><strong>{conversation.currentStage.replaceAll('-', ' ')}</strong></div><span className="sales-agent-live-dot">Step {conversation.progress?.current || 1} of {conversation.progress?.total || 7}</span></div><div className="sales-agent-message-list" aria-live="polite">{conversation.messages.map((message) => <MessageBubble key={message.id} message={message} />)}{isLoading && <div className="sales-agent-message-row is-agent"><div className="sales-agent-message-bubble sales-agent-loading" role="status"><span /><span /><span /></div></div>}{error && <div className="sales-agent-error" role="alert">{error}</div>}<div ref={messageEndRef} /></div>{!showLeadCapture && conversation.quickReplies?.length > 0 && <div className="sales-agent-quick-replies" aria-label="Suggested replies">{conversation.quickReplies.map((reply) => <button type="button" key={reply} onClick={() => sendMessage(reply)} disabled={isLoading}>{reply}</button>)}</div>}{showLeadCapture && <LeadCapturePanel onSubmit={submitLead} disabled={isLoading} />}{showRecommendations && <div className="sales-agent-recommendations">{conversation.recommendations.map((recommendation) => <ServiceRecommendationCard key={recommendation.serviceId} recommendation={recommendation} />)}</div>}{showCtas && <div className="sales-agent-cta-row"><a href={bookingHref} className="sales-agent-button sales-agent-button-gold" onClick={() => trackAssessmentEvent('sales_agent_strategy_call_clicked', { conversationId: conversation.id })}>Book Strategy Call <span aria-hidden="true">↗</span></a><button type="button" className="sales-agent-button" onClick={handleProposal}>Generate Initial Proposal <span aria-hidden="true">↗</span></button><a href="/#contact" className="sales-agent-button" onClick={() => trackAssessmentEvent('sales_agent_contact_clicked', { conversationId: conversation.id })}>Contact Us <span aria-hidden="true">↗</span></a></div>}<form className="sales-agent-composer" onSubmit={handleSubmit}><label className="sales-agent-sr-only" htmlFor="sales-agent-message">Message NOVAHAUS AI Sales Consultant</label><input id="sales-agent-message" value={input} onChange={(event) => setInput(event.target.value)} placeholder="Tell us what you are working on..." disabled={isLoading || showLeadCapture} /><button type="submit" aria-label="Send message" disabled={isLoading || showLeadCapture || !input.trim()}>Send <span aria-hidden="true">↗</span></button></form><div className="sales-agent-chat-footer-actions">{conversation.history?.length > 0 && <button type="button" onClick={goBack}>← Back</button>}<button type="button" onClick={handleReset}>Restart</button></div></div>
    </section>
  </main></div>
}
