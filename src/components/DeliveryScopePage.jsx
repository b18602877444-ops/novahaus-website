...
import { getPortfolioServiceById, getServicePortfolio, serviceCategories } from '../data/servicePortfolio.js'
...
function DeliveryScopePage({ InternalPage, SectionLabel, Reveal, ArrowIcon }) {
  const services = getServicePortfolio()
  const requestedService = new URLSearchParams(window.location.search).get('service')
  const requestedPortfolioService = getPortfolioServiceById(requestedService)
  const [selectedId, setSelectedId] = useState(() => requestedPortfolioService?.id || services[0].id)
  ...
  return <InternalPage eyebrow="NOVAHAUS / Delivery Scope" title={<>Clear scope.<br /><em>Better decisions.</em></>} description="What NOVAHAUS delivers, what the client provides and what requires custom quotation — in one shared reference."><section className="delivery-scope-page" aria-label="NOVAHAUS Delivery Scope Center">
    ...
    <section className="delivery-scope-operations-overview" aria-labelledby="delivery-operations-title"><Reveal><SectionLabel number="02">Monthly Operations Departments</SectionLabel><h2 id="delivery-operations-title">Defined capacity.<br /><em>Visible boundaries.</em></h2><p>Each department is a monthly operating function with a stated capacity, client responsibilities and custom-quote boundary.</p></Reveal><div className="delivery-scope-operations-grid">{growthOperationsPlans.map((plan) => <Reveal key={plan.id} className="delivery-scope-operations-card"><span>{plan.monthlyPrice}</span><h3>{plan.name}</h3><p>{plan.bestFor}</p><ul>{plan.monthlyStandardCapacity.slice(0, 4).map((item) => <li key={item}>{item}</li>)}</ul><strong>Onboarding: {plan.onboardingFee}</strong></Reveal>)}</div></section>
    <section className="delivery-scope-category-overview" aria-label="NOVAHAUS service categories"><Reveal><SectionLabel number="03">Five growth layers</SectionLabel><div className="delivery-scope-category-grid">{serviceCategories.map((category) => <div key={category.id}><span>{category.number}</span><h3>{category.name}</h3><p>{category.description}</p></div>)}</div></Reveal></section>
    <section className="delivery-scope-selector" aria-labelledby="delivery-scope-selector-title"><Reveal><SectionLabel number="03">Choose a service</SectionLabel><h2 id="delivery-scope-selector-title">Find the scope<br /><em>behind the system.</em></h2></Reveal><div className="delivery-scope-tabs" role="tablist" aria-label="Delivery scope services">{services.map((item, index) => <button key={item.id} type="button" role="tab" aria-selected={service.id === item.id} className={service.id === item.id ? 'is-active' : ''} onClick={() => setSelectedId(item.id)}><span>{String(index + 1).padStart(2, '0')}</span><small>{item.categoryName}</small>{item.name}</button>)}</div></section>
